...
**涉及模块**：`cqrs_router.py`（CQRSRouter + CacheStats）、`app.py`（review 流程集成）
...
**涉及模块**：`circuit_breaker.py`（BreakerPool）、`fallback_chain.py`（ChainResult）、`app.py`（per-language breaker routing）
...
**涉及模块**：`vector_store.py`（VectorStore + PatternMatch）、`app.py`（检索 + 存储集成）
...
│   ├── scripts/                    # 六个核心脚本，每个 <200 行
│   │   ├── app.py                  # CLI 入口与编排（~191 行）
