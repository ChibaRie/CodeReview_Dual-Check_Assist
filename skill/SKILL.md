---
name: dualcheck_code
description: AI 代码评审「双检」助手 — 静态快检 + AI 语义深检，CQRS 缓存，按语言隔离的熔断器池，舱壁隔离，向量记忆，质量趋势报告，可视化 Web UI。当用户说"搭建双检系统""初始化代码评审""安装dualcheck""建一个AI review系统""配置代码审查工具""帮我搭评审环境""给项目加自动评审"时使用本技能。也适用于将现有项目集成自动化代码评审的需求。
---
...
一套完整的 AI 代码评审系统。核心策略：静态规则抓表面问题，AI 解释告警并发现静态层漏掉的逻辑 Bug——两条路径互补，合并后给出三路对比。**v1.0.1 新增可视化 Web UI**（React + Vite 前端 + FastAPI 后端），`npm run dev` 一键启动。
...
```bash
cd <目标目录>/tests && python -m pytest -q   # 全部通过（218+）即部署成功
```

测试覆盖 `test_config.py`（9 用例）+ `test_reviewer.py`（3 用例）+ `test_fallback_chain_api_key.py`（4 用例）+ `test_reviewer_api_key.py`（4 用例）+ `test_web_server.py`（42 用例）+ 原有 156 基线用例 = 218 tests。详见 `tests/test_record.md`。
...
- **Web UI 独立于 Skill 包**：`web_server.py` 与 `web/` 位于项目根目录，不属于 `skill/` 分发包。Web UI 是本项目的工程产物，skill 保持纯净可安装。
