...
1. 先读轻量入口：`AGENTS.md`、`system/status.md`、`system/review-rules.json`。
2. `SYSTEM_SPEC.md` 是完整规约，不是每次评审都要全文装入的素材库。
3. 查缓存时先读 `system/status.md` 的缓存命中率概览，再决定是否深入 `data/.cache/`。
4. 查测试基线时先看 `tests/test_record.md` 的索引结构，只打开相关测试条目。
...
- 当前支持语言：`python`、`java`、`go`、`javascript`。
- 宁可先快审，不要让代码合并不经评审。
- 高频 Bug 模式要继续进入蒸馏区，变成可复用资产。
...
| 维度 | 健康 | 不健康 | 措施 |
| --- | --- | --- | --- |
| 缓存命中率 | >50% | <20% 或缓存膨胀 | 调整 TTL |
| 熔断器状态 | 全部 CLOSED | 频繁 OPEN 或振荡 | 检查 API Key/网络；振荡增大 cooldown |
| 发现质量 | AI 新发现 10-50% | AI 全确认或全否定 | 调整 prompt/规则精度 |

1. 先运行或读取最近 `system/status.md`。
2. 用缓存命中率、熔断器状态、静态/AI 发现比作为基础数据。
3. 将结果写入 `system/reports/YYYY-MM-DD-HHMMSS-health-check.md`。
4. 如果健康检查规则变化，同步更新 `system/review-rules.json`。

## 蒸馏流程

1. 从历史评审记录中提取反复出现的 Bug 模式。
2. 写入 `distill/patterns/`，一张卡片只讲一个 Bug 模式。
3. 卡片必须包含：核心观点、证据、可复用检查方法、触发条件。
4. 跨语言通用模式写入 `distill/patterns/universal/`，语言特定模式写入 `distill/patterns/<lang>/`。

## 完成后动作

1. 如果评审了新代码，将关键发现写入 `system/logs/YYYY-MM.md`。
2. 如果改动了系统功能、规则、脚本或目录结构，追加到 `system/update-logs/YYYY-MM.md`。
...
3. 在 `static_check.py` 中添加对应的 `_<lang>_check()` 函数。
4. 在 `data/` 下添加该语言的样本代码。
